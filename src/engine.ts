...
- council_review : 自分の候補（設計・計画・SQL・デプロイ手順）を GPT と Gemini に敵対的レビューさせ、欠陥・リスク・抜けを重大度付きで洗い出す検証ゲート（高リスク時のコミット/実行直前にだけ使う）
...
- ai_council / ask_gpt / ask_gemini / council_review を呼ぶのは次の高リスク時だけ:
  (a) 後戻りコストが高い設計の分岐  (b) 本番デプロイの直前  (c) 契約書・課金・セキュリティ・SQLに関わる判断。
  通常作業では呼ばない（コスト3倍・速度低下・弱いモデルへの引きずられを避ける）。
- 上記の高リスクでは「コミット/実行する前」に council_review で自分の候補（設計・計画・SQL・デプロイ手順）を敵対的検証する。確信が持てる難所ほど一度疑う。検証は別の脳に任せると自己批判より強い。これが IGSH が単一の脳（Claude単独）を超える核心。
- 外部AIの指摘・回答は各項目に「採用/棄却＋理由」を必ず付けて統合し、最終判断は自分（Claude）が下す。多数決にしない（弱い指摘・誤検出は理由付きで棄却してよい。実コード・実環境と矛盾する指摘は現物を優先）。
- 応答は日本語で、結論を先に、簡潔に。
...
const HELP_TEXT = `スラッシュコマンド
  ...
  /cwd         現在の作業フォルダ（と検出プロジェクト）を表示
  /stop        実行中のエージェントを中断
  /help        この一覧
  exit         終了
上記以外の /コマンド はそのままエンジン（Claude Code 本体・スキル全部ON）へ送られます
（例: /code-review /security-review /init 等の Code 流コマンド/スキルが使える）。
...
export function createEngine(opts: {
  ...
  const runSlash = async (t: string): Promise<boolean> => {
    ...
    switch ((cmd || "").toLowerCase()) {
      ...
      case "cwd":
        out(`作業フォルダ: ${cwd}${here ? `（検出プロジェクト: ${here.name}）` : "（登録プロジェクトの外）"}`);
        break;
      case "stop":
      case "interrupt": {
        try {
